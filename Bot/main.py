...
# Bot settings
allowed_roles: list[int] = [1266201501924331552]
sky_guardians_role_id: int = 1274471871873548381
owner_id: int = 371350209160019970
support_category_id = 1274471584551141376
max_teams = 4
...
async def unlockteam(interaction: disc.Interaction, member: disc.Member):
    ...
    await interaction.response.send_message(f"Team {team_data['emoji']} unlocked!")
                    ...
                    if user_id not in team["members"]:  # Only add if not already in the team
                        ...
                        if len(team["members"]) < team["max_members"]:
                            ...
                            if team_leader_id not in full_team_cooldowns or time.time() - full_team_cooldowns[team_leader_id] > cooldown_period:
                                await channel.send(
                                    f"The team of {client.get_user(team_leader_id).mention} :{team['emoji']}:\n"
                                    f"is full - Considering joining another team! {user.mention}"
                                )
                        ...
                        for member_id in team["members"]:
                            try:
                                ...
                                print(f"An error occurred while fetching user {member_id}: {e}")
